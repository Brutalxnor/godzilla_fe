...
export default function ProfileHeader({ data }: { data: ProfileCore }) {
  ...
  return (
    <div className="flex items-start gap-4">
      {/* Avatar / initials */}
      <div className="flex h-16 w-16 items-center justify-center rounded-full bg-[var(--chip)] text-lg font-semibold text-[var(--foreground)] overflow-hidden">
        {data.avatar_url ? (
...
function KeyVal({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-xl border p-3 bg-[var(--surface)] border-[var(--border)]">
