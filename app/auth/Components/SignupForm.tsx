...
export const SignupForm = () => {
  ...
  const onSubmit = async (data: SignUpFormData) => {
    try {
      ...
      const res = await fetch("https://gdv8tql1h2.execute-api.eu-west-2.amazonaws.com/api/v1/auth/register", {
        method: "POST",
      ...
      <div className="w-[60pc] bg-white rounded-3xl shadow-2xl border border-gray-100 overflow-hidden overflow-y-scroll">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="px-8 pb-8 space-y-5">
          ...
          <button
            type="submit"
            disabled={isSubmitting}
            className={`w-full p-4 font-semibold rounded-xl transition-all duration-300 shadow-lg focus:outline-none focus:ring-4 focus:ring-red-500/30 ${
              isSubmitting
