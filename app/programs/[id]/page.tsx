...
// import Sidebar from "@/app/components/shared/sidebar";
// import axios from "axios";
// import { Award, Calendar, MoveLeft, Star, Users, Play } from "lucide-react";
// import { use, useEffect, useMemo, useState } from "react";
// import { toast } from "react-toastify";
// import { Program } from "../components/programCard";
// import useGetUser from "@/app/Hooks/useGetUser";
...
//     "Overview" | "Materials" | "Reviews" | "FAQ"
//   >("Overview");
//   const [programMedia, setProgramMedia] = useState<
//     {
//       id: string;
...
//         setLoading(true);
//         const { data } = await axios.get(
//           `https://godzilla-be.vercel.app/api/v1/programs/${id}/media`,
//           {
//             headers: {
//               Authorization: `Bearer ${userDB?.data.access_token}`,
//             },
...
//               {tabs.map((tab) => {
//   const isMaterialsLocked = tab === "Materials" && !isSubscribed;

//   return (
//     <button
//       key={tab}
//       onClick={() => {
//         // 🚫 Block Materials if not subscribed
//         if (isMaterialsLocked) {
//           toast.info(
//             "You are not subscribed in this program. Subscribe now to access materials."
//           );
//           return;
//         }
...
//       }}
//       className={`
//         py-3 sm:py-4 px-2 border-b-2 font-medium whitespace-nowrap
//         ${
//           activeTab === tab
//             ? "border-red-600 text-red-600"
//             : "border-transparent text-gray-500 hover:text-gray-700"
//         }
//         ${isMaterialsLocked ? "text-gray-400 cursor-not-allowed hover:text-gray-400" : ""}
//       `}
//     >
//       {tab}
//     </button>
...
const Page = ({ params }: { params: Promise<{ id: string }> }) => {
  ...
  // ✅ SAME SUBSCRIBE LOGIC AS PROGRAMS PAGE
  const handleSubscribe = async () => {
    if (!userId) {
      toast.info("Please log in to subscribe to this program.");
      return;
    }
    ...
    try {
      ...
      // Re-fetch subscriptions to refresh `isSubscribed`
      const { data: subsRes } = await axios.get(
        `https://godzilla-be.vercel.app/api/v1/subscripe/${userId}`
      );
      ...
      >
        <div className="mx-auto w-full max-w-6xl px-4 sm:px-6 lg:px-0 py-4 md:py-8">
          ...
          <div className="px-0 sm:px-6 py-6">
            ...
            {/* ✅ Subscribe Button with REAL subscription logic */}
            <button
              disabled={isSubscribed}
